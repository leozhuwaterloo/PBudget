...
import { effectiveDays, matchesDayOfMonth } from "../dayofmonth";
...
export function matchesCondition(c: VendorCondition, txn: MatchTxn): boolean {
  ...
  const domDays = effectiveDays(c.daysOfMonth, c.dayOfMonth);
  if (domDays.length) preds.push(matchesDayOfMonth(domDays, txn.datetime));
  if (c.paymentChannel)
