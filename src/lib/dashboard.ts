...
export type DashboardData = {
  ...
  vendors: { key: string; name: string; icon: string | null; spend: number }[]; // (d) selected month
};
...
export async function dashboardData(userId: string, month?: string): Promise<DashboardData> {
  ...
  const vAgg = new Map<string, { name: string; icon: string | null; spend: number }>();
  for (const e of effective) {
    if (!inSelMonth(e.date) || isExcluded(e.categoryName)) continue;
    const key = e.vendorId ?? e.vendorName;
    const cur = vAgg.get(key) ?? { name: e.vendorName, icon: e.vendorIcon, spend: 0 };
    cur.spend += e.amount;
    vAgg.set(key, cur);
  }
